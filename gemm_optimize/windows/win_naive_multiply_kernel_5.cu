...
constexpr int BM = 32;
constexpr int BN = 32;
// 与第四版相同：BK=64 会使共享内存约翻倍，在多数 GPU 上降低每 SM 并发块数，实测反而明显变慢，故第五版保持 BK=32。
constexpr int BK = 32;
constexpr int TM = 4;
constexpr int TN = 4;
...
// 相对第四版：对齐且越界安全时经 __ldg 走只读纹理/L1 路径，减轻全局流量对 L2 的压力（依架构有收益）。
__device__ inline void store_a_tile(int (*As)[BK + 1], int ar, int ac, int rr, int cc, int m, int k,
                                    const int* __restrict__ A) {
    if (rr < m && cc + 3 < k) {
        const int* p = &A[static_cast<size_t>(rr) * k + cc];
        int4 v;
        v.x = __ldg(p + 0);
        v.y = __ldg(p + 1);
        v.z = __ldg(p + 2);
...
__device__ inline void store_b_tile(int (*Bs)[BN + 1], int br, int bc, int rr, int cc, int k, int n,
    ...
    if (rr < k && cc + 3 < n) {
        const int* p = &B[static_cast<size_t>(rr) * n + cc];
        int4 v;
        v.x = __ldg(p + 0);
        v.y = __ldg(p + 1);
        v.z = __ldg(p + 2);
...
/*
& "C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v13.1\bin\nvcc.exe" win_naive_multiply_kernel_5.cu -o naive_win_5_release.exe -ccbin "C:\Program Files\Microsoft Visual Studio\18\Community\VC\Tools\MSVC\14.50.35717\bin\Hostx64\x64" -std=c++14 -Xcompiler "/utf-8 /EHsc /O2 /MD" -Xptxas -O3 -allow-unsupported-compiler
*/
