...
ctxsw="--cpuctxsw=none"
output="nsys_easy"
# cuda_gpu_sum needs CUPTI kernel tables (often missing on WSL2); cuda_api_sum still lists API timings.
report="cuda_gpu_sum,cuda_api_sum"
...
usage() {
    ...
    echo "  -r report  : Comma-separated reports (default: cuda_gpu_sum,cuda_api_sum)"
    exit 1
}
...
# Run the nsys profile command (--cuda-memory-usage helps mem-related CUDA events in the export)
nsys profile $trace $sample $ctxsw --cuda-memory-usage=true --force-overwrite=true -o $output "$@"
